...
import { DeleteCircleForm } from "./delete-form";

type DeleteCirclePageProps = {
...
export async function generateMetadata({
  params,
}: DeleteCirclePageProps): Promise<Metadata> {
  const { locale } = await params;
  const t = await getTranslations({ locale, namespace: "admin.circles" });
  return { title: t("deleteTitle") };
}

export default async function DeleteCirclePage({ params }: DeleteCirclePageProps) {
