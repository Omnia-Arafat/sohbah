-- Optional per-circle capacity: once N students have taken a place in today's
-- queue, join_circle() refuses a new one until a spot is freed.
--
-- Deliberately not a stored counter. Capacity is checked against a live
-- count of today's attendance_records, so removing a student (already a
-- plain DELETE, see session-client.tsx's remove()) frees their slot for free
-- — there is nothing to keep in sync and nothing that can drift out of sync.
-- A slot is "occupied" for the whole session once taken, not just while the
-- student is still waiting: finishing recitation does not free it, only
-- being removed does.
--
-- Nullable, no default: every existing circle keeps registering students
-- exactly as before — a null max_students is unlimited, unchanged behaviour.

alter table public.circles
  add column max_students int check (max_students is null or max_students > 0);

-- =============================================================================
-- circle_public_info: the student-facing page needs to know the cap (and see
-- it change) before anyone attempts to join, not just get a rejection.
-- =============================================================================
...
-- =============================================================================
-- join_circle: the actual enforcement point.
-- =============================================================================
...
begin
  ...
  -- start of a session cannot collide on queue_order — and, now, cannot both
  -- slip into the last open seat either: the capacity check below runs inside
  -- the same lock, so it sees a count that cannot change under it.
