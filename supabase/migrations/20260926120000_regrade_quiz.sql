-- Re-mark a quiz's finished attempts after its questions change.
--
-- Staff can now edit a question — its wording, options, correct answer or
-- points — after students have already sat the quiz. Without this their saved
-- marks would keep describing the old key, and a deleted question would stay in
-- `max_score`. This grades every finished attempt again against the paper as it
-- stands now, with the same rules `submit_quiz_attempt` uses.
--
-- Written answers (short_text) keep the mark a معلمة gave them: nothing about
-- an edit makes a person's judgement automatically wrong.
--
...
begin
  ...
  -- A written answer's mark cannot exceed its question's current points.
  update public.quiz_answers qa
