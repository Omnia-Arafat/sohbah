-- A supervisor could not run a session she is supposed to supervise.
--
-- Every write to `attendance_records` was limited to the circle's own teacher
-- or an admin, so a مشرفة opening the queue could see it and nothing more:
-- removing a student came back "تعذر إتمام العملية", and marking who recited
-- or reordering the queue would have failed the same way.
--
-- Following the queue and marking who recited is the whole job, so all three
-- writes now accept a supervisor of that circle's academy.
...
-- The manual reorder runs as SECURITY DEFINER, so RLS never sees it — it
-- carries its own copy of the same rule, and needs the same widening.
